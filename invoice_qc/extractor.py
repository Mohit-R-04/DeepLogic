...
def _extract_text_from_pdf(pdf_path: Path) -> str:
    """read all text from pdf, join pages (using PyPDF2 now)"""
    text_parts = []
...
def _find_last(pattern: str, text: str, flags=0) -> Optional[str]:
    """return last match group for regex or None"""
...
def parse_invoice_from_text(text: str, filename: str = "") -> Invoice:
    ...
    # use _find_last so we are more likely to hit final totals, not some random one above
    net_str = _find_last(r"(?:Net\s*Total|Netto|Gesamtwert)[^\d]*([\d\.,]+)", text)
    if net_str:
        net_total = _parse_float(net_str)

    tax_str = _find_last(r"(?:Tax|MwSt)[^\d]*([\d\.,]+)", text)
    if tax_str:
        tax_amount = _parse_float(tax_str)

    gross_str = _find_last(r"(?:Total\s*Amount|Brutto|Gesamtwert inkl\. MwSt\.?)[^\d]*([\d\.,]+)", text)
    if gross_str:
    ...
    # try to parse line items (super simple)
    ...
    if items_text:
        ...
        for ln in ilines:
            ...
            # only parse lines that start with a digit (position)
            if not re.match(r"^\s*\d+", ln):
                continue

            parts = ln.split()
            if len(parts) < 4:
                # not enough stuff here, probably broken line
                continue

            # remove the first part (position number)
            pos_token = parts[0]
            rest = parts[1:]
